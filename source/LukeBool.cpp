...
int LukeBool::IntToBoolVec(bool* out, int I, int m) {
    /*
...
int LukeBool::Inner(bool* x1, bool* x2, int len) {
    int out = 0;
    for(int i = 0; i < len; i++) {
        out += (x1[i]&&x2[i]);
    }
...
void LukeBool::print(bool* x, int len, const char* pre) {
    if(pre) cout << pre;
    for(int i = 0; i < len; i++) cout << x[len-i-1];
    cout << endl;
}

void LukeBool::BitwiseAnd(bool* x1, bool* x2, bool* out, int len) {
    for(int i = 0; i < len; i++) {
        out[i] = x1[i]*x2[i];
    }
}

void LukeBool::BitwiseXor(bool* x1, bool* x2, bool* out, int len) {
    for(int i = 0; i < len; i++) {
        out[i] = x1[i]^x2[i];
    }
}

void LukeBool::copy(bool* src, bool* dst, int len) {
    for(int i = 0; i < len; i++) {
        dst[i] = src[i];
    }
}

bool LukeBool::increment(bool* x, int len) {
    bool out;
    int i = 0;
    bool exit;
    do {
...
void LukeBool::zeros(bool* x, int len) {
    for(int i = 0; i < len; i++) {
        x[i] = false;
    }
}

bool LukeBool::nextUniquePerm(bool* out, const bool* in, int len) {
    bool outB = false;
    int W = 0;
    for(int i = 0; i < len; i++) {
        W += in[i];
...
int LukeBool::Weight(bool* x, int len) {
    int out = 0;
...
void LukeBool::fromString(bool* out, const char* in_s) {
    int n = strlen(in_s);
    for(int i = 0; i < n; i++) {
        if(in_s[i]=='0') {
            out[i] = 0;
...
int LukeBool::ReedDecoder(bool* x, int R, int M, bool* c, bool* e) {
    /*
    ...
    for(int r = R; r >= 0; r--) {
        ...
        do {
            ...
            for(int v = 0; v < (int)pow(2,M-r); v++) {
                ...
                LukeBool::IntToBoolVec(v_vec, v, M-r);
                LukeBool::print(v_vec,M-r,"v: ");
                ...
                if(r) {
                    for(int t = 0; t < (int)pow(2,r); t++) {
                            ...
                            LukeBool::IntToBoolVec(t_vec,t,r);
                            LukeBool::print(t_vec,r,"t: ");
                            ...
                            int k = LukeBool::BoolVecToInt(k_vec, M);
                            this_vote += b[k];
            ...
            if(votes > (int)(0.5*pow(2,M-r))) {
                for(int i = 0; i < n; i++) {
                    ...
                    LukeBool::IntToBoolVec(i_vec,i,M);
                    bool this_f = 1;
                    for(int j = 0; j < M; j++) {
            ...
            exit = !LukeBool::nextUniquePerm(temp,I,M);
            LukeBool::copy(temp,I,M);
            delete [] temp;
        } while(!exit);

        // Construct the order r sub-code word, b_r, and subtract it from b
        bool* temp = new bool[n];
        LukeBool::BitwiseXor(b,b_r,temp,n);
        LukeBool::copy(temp,b,n);
        LukeBool::print(b,n,"new b: ");
        if(c) {
            LukeBool::BitwiseXor(c,b_r,temp,n);
            LukeBool::copy(temp,c,n);
        }
        delete [] temp;
