...
BMSparse BMSparse::col(const int j) const {
    ...
    if(j<m) {
        ...
        for(BM_Data::iterator it = data->begin(); it != data->end(); it++) {
            int thisI = *it;
            int this_i, this_j;
            //if((thisI<0)||(thisI>maxI())) cout << "Inside col 1. thisI = " << thisI << endl;
            IToij(thisI, this_i, this_j);
            //if((thisI<0)||(thisI>maxI())) cout << "Inside col 2" << endl;
            if(this_j==j) {
                out.E(this_i,0,1);
            }
        }
        ...
        while((!exit)&&((k+e)<m)) {
            ...
            for(BM_Data::iterator it = out.data->begin(); (pivot_i==n)&&(it != out.data->end()); it++) {
                ...
                if((this_i>=k)&&(this_j==(k+e))) {
                    pivot_i = this_i;
                    //cout << "Pivot found: i = " << pivot_i << endl;
                    //out.printFull();
                }
            }
            if(pivot_i < n) {
                ...
                //cout << "Rows swapped: i1 = " << k << ", i2 = " << pivot_i << endl;
                //out.printFull();
                BMSparse this_out;
                this_out.assign(out);
                for(BM_Data::iterator it = out.data->begin(); it != out.data->end(); it++) {
                    ...
                    int this_i, this_j;
                    out.IToij(thisI, this_i, this_j);
                    //cout << "pivot = " << pivot_i << ", k = " << k << ", i = " << this_i << ", j = " << this_j << endl;
                    if((this_i>k)&&(this_j==(k+e))) {
                        {
                            BMSparse tempout = this_out.addRows(this_i,k);
                            this_out.assign(tempout);
                        }
                        //cout << "Row added: s = " << k << ", t = " << this_i << endl;
                        //this_out.printFull();
                    }
                }
                out.assign(this_out);
...
BMSparse BMSparse::elementaryFactor() const {
    ...
    */
    ...
    for(int k = 0; k < n; k++) {
        ...
        //cout << "n = " << n << "; m = " << out.m << "; k = " << k << "; col = " << col << endl;
        if(thisP%2) {
            out.E(k,col,1);
            col++;
        }
...
BMSparse BMSparse::minimalFactor(bool verbose) const {
    if(verbose) cout << "mininalFactor BEGIN" << endl;
    BMSparse out;
    ...
    if(verbose) cout << "exit condition: thisB.m = " << (this_rank+this_delta) << endl;

    int iteration = 0;
    while((!exit)/*&&(!thisNullspace.isEmpty())*/) {
        if(verbose) cout << "iteration = " << iteration << endl;
        if(verbose) cout << "thisB.m = " << thisB.m << endl;
        if(verbose) cout << "thisB" << endl;
        if(verbose) thisB.printFull();
        if(verbose) cout << "thisNullspace" << endl;
        if(verbose) thisNullspace.printFull();
        BMSparse thisIV = thisNullspace.sub(0,0,-1,1);
        if(verbose) cout << "thisIV" << endl;
        //if(verbose) thisIV.printFull();
        ...
        if(verbose) cout << "thisF" << endl;
        if(verbose) thisF.printFull();
        if(verbose) cout << "thisG" << endl;
        if(verbose) thisG.printFull();
        if(thisG.m%2) {
            BMSparse tempG = thisG && BMSparse(n,1);
            thisG.assign(tempG);
        }
        if(verbose) cout << "thisZ" << endl;
        if(verbose) thisG.printFull();
        BMSparse this_x = thisF.sub(0,0,-1,1) + thisG.sub(0,0,-1,1);
        if(verbose) cout << "this_x" << endl;
        if(verbose) this_x.printFull();
        ...
        if(verbose) cout << "thisZ^" << endl;
        if(verbose) thisG.printFull();
        if(verbose) cout << "!thisF_1" << endl;
        if(verbose) thisF.sub(0,1,-1,-1).printFull();
        if(verbose) cout << "!thisZ^_1" << endl;
        if(verbose) thisG.sub(0,1,-1,-1).printFull();
        if(false&&thisF.isEmpty()&&(!thisG.isEmpty())){
            if(verbose) cout << "QWER" << endl;
            BMSparse tempB = thisG.sub(0,1,-1,-1);
            if(verbose) cout << "asdf" << endl;
            thisB.assign(tempB);
            if(verbose) cout << "zxcv" << endl;
        } else {
            if(verbose) cout << "QWER" << endl;
        ...
        if(false&&thisF.isEmpty()&&(!thisG.isEmpty())) {
            if(verbose) cout << "tyui" << endl;
            BMSparse tempNullspace(0,0);
            if(verbose) cout << "gjhk" << endl;
            thisNullspace.assign(tempNullspace);
            if(verbose) cout << "bnm," << endl;
        } else {
            if(verbose) cout << "tyui" << endl;
    ...
    if(verbose) cout << "Reduction complete!" << endl;
    if(verbose) cout << "thisB" << endl;
    if(verbose) thisB.printFull();
    if(verbose) cout << "thisNullspace" << endl;
    if(verbose) thisNullspace.printFull();
    ...
    if(verbose) cout << "mininalFactor END" << endl;
    return out;
}
...
BMSparse BMSparse::random(const int inN, const int inM, const double in_P) {
    ...
    for(int k = 0; k < (in_P*inN*inM); k++) {
        int this_I = LukeInt::randi(0,inN*inM-1);
        int i,j;
...
void BMSparse::LEUKTrapezoidal(BMSparse& outL, BMSparse& outE, BMSparse& outU, BMSparse& outK, int height, int order) const {
    ...
    for(int k = 0;(true)&&(k < (int)fmin(fmin(n,m),height)); k++) {
        ...
        if(order==0) { // Find pivot based on large amount of activity
            ...
            pivot_j = LukeInt::randi(k+e,m-1);
            vector<int> temp_is;
            for(int i = k; i < n; i++) {
                if(outU.E(i,pivot_j)) temp_is.push_back(i);
            }
            pivot_i = temp_is.at(LukeInt::randi(0,temp_is.size()-1));
        }
        ...
        //cout << "outU after selecting pivot" << endl;
        //outU.printFull();
        ...
        outU.assign(outU_temp);
        //cout << "outU after Gaussian elimination" << endl;
        //outU.printFull();
...
int BMSparse::EUKPentagonal(BMSparse& outE, BMSparse& outU, BMSparse& outK, int height) const {
    ...
    // Find a vector in the nullspace of submatrix B with weight of 3 (or nearest value >= 3)
        ...
        //BMSparse this_N = outU.nullspace();
        ...
        while(!found_v&&(this_x.sum())) {
            ...
            //int this_w = this_nv.sub(0,b_start_col,-1,-1).sum();
        ...
        if(found_v) {
            ...
            for(int i = c0; i < this_N.get_n(); i++) {
                if(this_v.E(i,0)) {
                    ...
                    //BMSparse thisK = BMSparse::perm(m,c0,i);
            ...
            //b_start_col += this_v.sub(0,b_start_col,-1,-1).sum();
        } else {
            b_start_col = m;
        }
