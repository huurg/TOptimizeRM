...
void Signature::h_decomposition(const int h, Signature& out_alpha, Signature& outAp, Signature& outB) const {
    // TODO: make outB actually n-1 qubits in size
...
Signature Signature::subsig(bool* inc_vec) const {
    ...
    int n_sub = LukeBool::Weight(inc_vec,n);
    out.set_n(n_sub);
    ...
    //LukeInt::print(this_sub_mapping,n);
    ...
    for(unordered_set<int>::iterator it = data_us->begin(); it != data_us->end(); it++) {
        ...
        bool exit = false;
        //LukeInt::print(this_inds,3,"this_inds: ");
        ...
        if(!exit) {
            //LukeInt::print(this_sub_inds,3,"this_sub_inds: ");
            switch(this_type) {
                case 0:
