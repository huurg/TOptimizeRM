...
void GateStringSparse::print(ostream& inOS) const {
    inOS << "Gate string: ";
    //for(unordered_set<int>::iterator it = data_us->begin(); it!=data_us->end(); it++) cout << *it << " ";
    //cout << endl;
...
GateStringSparse GateStringSparse::MonomialBF(bool* in_monvec, int in_m) {
    ...
    for(int i = 0; i < N; i++) {
        LukeBool::IntToBoolVec(B_i,i,in_m);
        if(LukeBool::Inner(B_i,in_monvec,in_m)==r) {
            out.set(i);
        }
    }
    return out;
...
GateStringSparse GateStringSparse::randomWeightN(int in_n, int N) {
    ...
    for(int i = 0; i < N; i++) {
        bool exit = false;
        while(!exit) {
            int this_I = LukeInt::randi(1,pow(2,in_n)-1);
            if(!out.E(this_I)) {
                out.set(this_I);
                exit = true;
            }
